...
class SampleApp
{
    ...
    {
        Console.WriteLine("[Client] Entering in Client");

        bool status;

        status = PadiDstm.Init();

        if (status)
            Console.WriteLine("[Client] Init OK");
        else
            Console.WriteLine("[Client] Init FAIL");

        status = PadiDstm.Status();

        if (status)
            Console.WriteLine("[Client] Servers Status OK");
        else
            Console.WriteLine("[Client] Servers Status FAIL");

        Console.WriteLine("[Client] Exiting in Client");
